...
localparam signed [15:0] YD =  16'sd8 - P_Y_OFFSET - 16'sd24;
...
always @(posedge clk) begin
	if (reset) begin
		...
		if (go) begin
			...
			render_y_latch <= (render_y == 9'd261) ? 9'd0 : render_y + 9'd1;
...
always @(posedge clk) begin
	if (reset) begin
		...
		// =============================================================
		S_IDLE: begin
			if (go_latch && render_y_latch < 9'd224) begin
		...
		S_B0_ROM0: begin
			begin
				...
				rom_addr <= {lat_code[15:0] & 16'h7FFF, py, 2'b00};
			end
			rom_req <= ~rom_req;
			st <= S_B0_ROM1;
		end
...
wire render_window = (render_x < 10'd288) && (render_y < 9'd224);
...
always @(posedge clk) begin
	...
	rd_win     <= render_window && (ox < 9'd288);
end
